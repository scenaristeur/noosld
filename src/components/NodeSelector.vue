  ...
  <b-form-group>
    <b-form-tags id="tags-with-dropdown" v-model="value" no-outer-focus class="mb-2">
      ...
      <template v-slot="{ tags, disabled, addTag, removeTag }">
        ...
        <!-- {{currentProp.values.filter(n => n.type == 'node').map(n => n['@id'])}}  -->
...
export default {
  ...
  computed: {
    ...
    availableOptions() {
      ...
      const options = y_store.todos.filter(opt => this.value.indexOf(opt['ve:name']) === -1)
    ...
    searchDesc() {
      if (this.criteria && this.availableOptions.length === 0) {
        return 'There are no todos matching your search criteria'
      }
      return ''
    },
    // todos() {
    //   return this.y_store.todos;
    // }
  },
  methods: {
